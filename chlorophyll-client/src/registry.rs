...
pub fn dispatch(registry: &mut Registry, packet: &Packet, now: DateTime<Utc>) -> Option<Reading> {
    let id = packet.id();
    let device = registry.devices.entry(id).or_insert_with(|| DeviceInfo { id, ..Default::default() });
    ...
    match packet.command() {
        ...
        PacketCommand::DataReading(data) => {
            ...
            Some(Reading { sensor_id: id, kind, value, at: now })
        }
...
mod tests {
    ...
    fn packet_roundtrips_through_postcard() {
        let packet = Packet::new(PacketCommand::DataReading(DataType::Temperature(Celsius::new(21.5))), 42);
    ...
    fn dispatch_updates_registry_and_emits_reading() {
        ...
        let temp = Packet::new(PacketCommand::DataReading(DataType::Temperature(Celsius::new(22.0))), 7);
        ...
        let hum = Packet::new(PacketCommand::DataReading(DataType::RelativeHumidity(RelativeHumidity::new(55.0))), 7);
        dispatch(&mut registry, &hum, now);

        let light = Packet::new(PacketCommand::DataReading(DataType::Light(Lux::new(123.0))), 7);
